...
test.describe("tray module - RPC layer", () => {
  ...
  test("should call tray.setToolTip via RPC", async () => {
    ...
    await window.evaluate(async () => {
      const rpc = (window as any).rpc;
      await rpc.call("tray.setToolTip", "Test Tooltip");
    });
  ...
  test("should call tray.getTitle via RPC", async () => {
    const window = electronApp.windows()[0];
  ...
  test("should call tray.setTitle via RPC", async () => {
    const window = electronApp.windows()[0];
    ...
    await window.evaluate(async () => {
      const rpc = (window as any).rpc;
      await rpc.call("tray.setTitle", "Test Title");
    });
  ...
  test("should call tray.getBounds via RPC", async () => {
    const window = electronApp.windows()[0];
  ...
  test("should call tray.isDestroyed via RPC", async () => {
    const window = electronApp.windows()[0];
    ...
    await window.evaluate(async () => {
      const rpc = (window as any).rpc;
      await rpc.call("tray.isDestroyed");
    });
  ...
  test("should call tray.setContextMenu via RPC", async () => {
    const window = electronApp.windows()[0];
    ...
    await window.evaluate(async () => {
      const rpc = (window as any).rpc;
      await rpc.call("tray.setContextMenu", [{ label: "Item 1" }]);
    });
